...
### 2.5. AI Guides Integration
> **Strategy:** Auto-pull established patterns from AI guides to enrich PRP context

- **Check for AI Guides:**
  ```bash
  # Look for AI guides in project
  ls docs/AI_DOMAIN_TRANSLATION_GUIDE.md
  ls docs/AI_CAPABILITIES_AND_LIMITS.md
  ```

- **Extract Relevant Patterns:**
  - Read AI_DOMAIN_TRANSLATION_GUIDE.md
  - Identify patterns relevant to the feature (database, API, state, forms, etc.)
  - Extract gotchas and anti-patterns
  - Example: If feature needs database queries, pull Prisma patterns from guide

- **Extract Known Limits:**
  - Read AI_CAPABILITIES_AND_LIMITS.md
  - Identify relevant tool limits (file processing, API rate limits, etc.)
  - Include workarounds in PRP context
  - Example: If feature processes images, include known image size limits

- **Format for PRP:**
  ```yaml
  # In "All Needed Context" section:
  project_patterns:
    - source: docs/AI_DOMAIN_TRANSLATION_GUIDE.md
      pattern: "Database queries use Prisma ORM with TypeScript types"
      gotcha: "Always use `include` for relations to avoid N+1"
      example: |
        const users = await prisma.user.findMany({
          include: { profile: true }
        })

    - source: docs/AI_CAPABILITIES_AND_LIMITS.md
      limit: "Image processing max 10MB per file"
      workaround: "Use chunked processing for larger files"
  ```

- **Benefits:**
  - PRP automatically follows established project patterns
  - Includes known gotchas upfront
  - Reduces need for clarifying questions
  - Faster implementation with project-specific context
...
7. **AI Guides Integration:** Auto-reference project patterns from guides
8. **Project-Specific Context:** Include established conventions from AI_DOMAIN_TRANSLATION_GUIDE.md
